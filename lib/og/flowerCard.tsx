/**
 * Imagen de vista previa (WhatsApp, iMessage…) compartida por las rutas del ramo.
 */
...
/** Tarjeta de vista previa: girasol de luz, antetítulo, título manuscrito y una línea. */
export async function flowerCard({ kicker = KICKER, title, line = LINE }: { kicker?: string; title: string; line?: string }) {
  const name = title
  // Dos fuentes con nombre propio: si solo se carga la manuscrita, se cuela en el resto del texto.
  const [font, sans] = await Promise.all([googleFont('Ephesis', name), googleFont('Inter', kicker.toUpperCase() + line)])
    ...
    (
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', maxWidth: 620 }}>
          ...
          <div style={{ fontSize: name.length > 16 ? 92 : name.length > 11 ? 112 : 140, lineHeight: 1.05, color: '#ffd23f', fontFamily: font ? 'Ephesis' : 'serif', marginTop: 12 }}>
            {name}
          </div>
